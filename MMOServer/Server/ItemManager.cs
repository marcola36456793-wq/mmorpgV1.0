...
public ItemInstance CreateItemInstance(int templateId, int quantity = 1)
{
    ...
    return instance;
        ...
        public bool RemoveItemFromPlayer(string sessionId, int instanceId, int quantity = 1)
        {
            ...
            if (player == null)
            {
                Console.WriteLine($"❌ RemoveItem: Player not found");
                return false;
            }
            ...
            if (item.isEquipped)
            {
                Console.WriteLine($"❌ RemoveItem: Cannot drop equipped item");
                return false;
            }
            ...
            {
                SaveInventory(inventory);
                Console.WriteLine($"📤 {player.character.nome} dropped {quantity}x item {instanceId}");
            }
